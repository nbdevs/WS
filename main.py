...
def main():
    ...
    timespan = ["minute", "hour", "day"] # initialising array of time durations
    
    # data collection
    aapl_data = analyser.aapl_data_population(timespan[0], 5) #multiplier of 5 for 5 minute interval
    ...
    aapl_data_hourly = analyser.hourly_stock_prices(timespan[1], 1) #multiplier of 1 for 1 minute interval
    aapl_data_daily = analyser.daily_stock_prices(timespan[2], 1) #multiplier of 1 for 1 minute interval
    
    array_list_hourly = analyser.compute_OHLC(aapl_data_hourly)
    array_list_daily = analyser.compute_OHLC(aapl_data_daily)
