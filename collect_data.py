...
class Scraper:
    ...
    def pull_stock_data(self, tick, multiplier, timespan, date_to, date_from):
        ...
        # parsing request url
        url = "https://api.polygon.io/v2/aggs/ticker/{0}/range/{1}/{2}/{3}/{4}?adjusted=true&sort=asc&limit=5000".format(
            tick, multiplier, timespan, date_from, date_to)

        # setting session object python3 main.py
        session = r.session()

        agg_stock_bars = session.get(url, headers={'Authorization': '{0} {1}'.format(
            user, api_key), 'Accept': 'text/csv'})
